...
**核心理念**：每个 NPC 角色都由你指定的 AI 模型扮演。你可以为不同角色选择不同的 AI 模型 —— 用高性能模型驱动关键 NPC，用轻量模型驱动路人角色，完全由你掌控。游戏编辑器中为每个角色独立配置 AI 模型、人设 Prompt、性格特征和行为规则，AI 会根据这些设定即兴演绎角色，动态生成剧情、对话和选项。
...
| 类别 | 亮点 |
|------|------|
| **AI 引擎** | 每个 NPC 独立指定 AI 模型、流式响应、RAG 知识库、滚动摘要记忆、Token 预算管理、7 条结构化行为规则 |
...
#### 角色 AI 模型分配
- **每个角色独立指定 AI 模型**：在游戏编辑器中为每个 NPC / 叙述者单独选择 AI 模型
- 支持为不同角色使用不同模型（如高性能模型驱动 Boss，轻量模型驱动路人）
- 角色切换时自动切换对应的 AI 模型
- 每个角色拥有独立的人设 Prompt、性格、背景、目标和行为规则

#### 流式响应与上下文管理
- **SSE 流式解码**：AI 逐句生成，实时渲染到聊天界面
- **三级模型回退**：角色指定模型 → 全局选择 → 模型列表首项
...
| **角色** | **独立 AI 模型选择**、性别、角色类型（可扮演/NPC/叙述者）、职业、技能、性格/爱好/背景/目标、AI 设定 Prompt、纪律规则、惩罚/奖励偏好、训练课程 |
