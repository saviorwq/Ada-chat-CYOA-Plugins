...
- 约束工具箱完善：补充模板占位符、公理注入、`responseMode`、Hook 安全策略说明。
- 发布流程完善：发版入口、检查清单、产物说明统一到文档体系中。

## v2.2.1 补充说明（不变更版本号）

- 新增“剧情约束随机 NPC”能力：按当前章节+地点上下文随机补充临时 NPC，并写入存档态。
- 动态 NPC 会自动纳入在场人物事实框架与可互动面板，避免“只出现于文案、不出现在交互”的割裂感。
- 生成策略包含概率、冷却与数量上限控制，减少刷屏与叙事噪音。
- 同步保留近期修复：流式输出尾部去重，降低“选项先出现后跳删”的视觉突兀感。
...
- `release/cyoa_v2.2.1.zip`（Windows 友好）
- `release/cyoa_v2.2.1.tar.gz`（Linux/macOS 友好）
...
4. 小模型（7B~9B）建议：`temperature 0.10~0.20`，`top_p 0.70~0.95`。
